"""
PEP 517 build backend for relateby-pattern (single-crate: pattern-core only).
Produces a wheel that installs into the relateby namespace (relateby.pattern).
"""
...
COMPONENT = "pattern"
CRATE_DIR = "pattern-core"
WHEEL_NAME_PREFIX = "pattern_core"


def _repo_root() -> Path:
    """Repository root (pattern-rs)."""
    # This file is python/relateby-pattern/relateby_build/__init__.py
    return Path(__file__).resolve().parent.parent.parent.parent


def _project_dir() -> Path:
    """Project dir (python/relateby-pattern)."""
...
def build_wheel(
    ...
    dist_name = "relateby_pattern"  # PEP 427: hyphen -> underscore
    ...
    with tempfile.TemporaryDirectory() as stage:
        ...
        (native_dir / "__init__.py").write_text(
            "# Native extension(s): pattern_core\n", encoding="utf-8"
        )

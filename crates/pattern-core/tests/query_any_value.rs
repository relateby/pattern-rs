...
fn test_any_value_no_matching_values() {
    ...
    let pat = Pattern::pattern(
        5,
        vec![Pattern::point(10), Pattern::point(3)],
    );
...
fn test_any_value_large_flat_pattern() {
    ...
    let elements: Vec<Pattern<i32>> = (0..1000)
...
fn test_any_value_short_circuit_behavior() {
    ...
    assert!(counter.get() <= 3, "Expected early termination, but evaluated {} values", counter.get());
}

...
#[cfg(feature = "python")]
fn python_dict_to_value_map(py: Python, py_dict: &Bound<'_, PyDict>) -> PyResult<HashMap<String, Value>> {
    let mut map = HashMap::new();
...
fn collect_pattern_values(pattern: &PyPattern, result: &mut Vec<String>) {
    result.push(pattern.value.clone());
    for elem in &pattern.elements {
        collect_pattern_values(elem, result);
    }
}
...
fn filter_pattern_recursive(pattern: &PyPattern, predicate: &Bound<'_, PyAny>, result: &mut Vec<PyPattern>) {
...
fn find_first_pattern_recursive(pattern: &PyPattern, predicate: &Bound<'_, PyAny>) -> Option<PyPattern> {
    let py_pattern = PyPattern {
        value: pattern.value.clone(),
        elements: pattern.elements.clone(),
    };
...
fn indices_at_pattern_recursive(pattern: &PyPattern, path: &mut Vec<usize>) -> PyPattern {
    let path_str = format!("{:?}", path);
    let mut new_elements = Vec::new();
    for (i, elem) in pattern.elements.iter().enumerate() {
        path.push(i);
        new_elements.push(indices_at_pattern_recursive(elem, path));
        path.pop();
    }
    PyPattern {
        value: path_str,
...
fn to_rust_pattern(pattern: &PyPattern) -> Pattern<String> {
    Pattern {
        value: pattern.value.clone(),
        elements: pattern.elements.iter().map(|e| to_rust_pattern(e)).collect(),
    }
}
...
impl PyPattern {
    ...
    ///
    /// Args:
    ///     value: The value component (string, int, float, or bool)
    ///
    /// Returns:
    ///     Pattern: Atomic pattern instance
    #[staticmethod]
    fn point(_py: Python, value: &Bound<'_, PyAny>) -> PyResult<Self> {
        // Convert value to string representation for MVP
        let value_str = if let Ok(s) = value.extract::<String>() {
            ...
            return Err(type_error_to_python("Value must be string, int, float, or bool"));
        };
    ...
    /// Create a pattern with value and elements.
    ///
    /// Args:
    ///     value: The value component (string, int, float, or bool)
    ...
    ///
    /// Returns:
    ///     Pattern: Pattern instance with value and elements
    #[staticmethod]
    fn pattern(_py: Python, value: &Bound<'_, PyAny>, elements: Vec<PyPattern>) -> PyResult<Self> {
        let value_str = if let Ok(s) = value.extract::<String>() {
            s
        } else if let Ok(i) = value.extract::<i64>() {
            i.to_string()
    ...
    /// Create a pattern from list of values.
    ///
    ...
    ///
    /// Args:
    ///     value: The decoration value (string, int, float, or bool)
    ///     values: List of values to convert to atomic patterns
    ///
    /// Returns:
    ///     Pattern: Pattern instance with value as decoration and values as elements
    #[staticmethod]
    fn from_list(py: Python, value: &Bound<'_, PyAny>, values: &Bound<'_, PyList>) -> PyResult<Self> {
        let value_str = if let Ok(s) = value.extract::<String>() {
            s
        } else if let Ok(i) = value.extract::<i64>() {
            i.to_string()
        ...
        for item in values.iter() {
            elements.push(PyPattern::point(py, &item)?);
        }
    ...
    ///
    /// Returns:
    ///     int: Number of direct child patterns
    ...
    ///
    /// Returns:
    ///     list[str]: All values in pre-order (root first, then elements)
    fn values(&self) -> Vec<String> {
        let mut result = Vec::new();
        collect_pattern_values(self, &mut result);
        result
    }

    /// Check if any value satisfies the predicate.
    fn any_value(&self, py: Python, predicate: &Bound<'_, PyAny>) -> PyResult<bool> {
        let result = self.values().iter().any(|v| {
            match predicate.call1((v.clone(),)) {
    ...
    fn all_values(&self, py: Python, predicate: &Bound<'_, PyAny>) -> PyResult<bool> {
        let result = self.values().iter().all(|v| {
            match predicate.call1((v.clone(),)) {
                Ok(result) => result.extract::<bool>().unwrap_or(false),
                Err(_) => false,
            }
    ...
    fn matches(&self, other: &PyPattern) -> bool {
        if self.value != other.value || self.elements.len() != other.elements.len() {
            return false;
        }
        self.elements.iter().zip(other.elements.iter()).all(|(a, b)| a.matches(b))
    }

    /// Check if pattern contains other as subpattern.
    fn contains(&self, other: &PyPattern) -> bool {
        if self.matches(other) {
            return true;
        }
        self.elements.iter().any(|e| e.contains(other))
    }

    /// Transform values while preserving structure.
    fn map(&self, py: Python, func: &Bound<'_, PyAny>) -> PyResult<PyPattern> {
        let new_value = match func.call1((self.value.clone(),)) {
            Ok(result) => {
        ...
        let new_elements: Vec<PyPattern> = self.elements.iter()
            .map(|e| e.map(py, func))
    ...
    fn fold(&self, py: Python, init: &Bound<'_, PyAny>, func: &Bound<'_, PyAny>) -> PyResult<PyObject> {
        let mut acc = init.to_object(py);
        for value in self.values() {
            acc = func.call1((acc.bind(py), value))?.to_object(py);
        }
        Ok(acc)
    }

    /// Combine two patterns associatively.
    fn combine(&self, other: PyPattern) -> PyResult<PyPattern> {
        // For string values, concatenate
        let combined_value = format!("{}{}", self.value, other.value);
    ...
    fn depth_at(&self) -> PyResult<PyPattern> {
    ...
    fn size_at(&self) -> PyResult<PyPattern> {
        let size = self.size();
        let new_elements: Vec<PyPattern> = self.elements.iter()
            .map(|e| e.size_at())
            .collect::<PyResult<Vec<_>>>()?;
    ...
    fn indices_at(&self) -> PyResult<PyPattern> {
        Ok(indices_at_pattern_recursive(self, &mut Vec::new()))
    }
    ...
    fn validate(&self, rules: &PyValidationRules) -> PyResult<()> {
        ...
        rust_pattern.validate(&rules.rules)
    ...
    fn __repr__(&self) -> String {
        format!("Pattern(value={:?}, elements={})", self.value, self.elements.len())
    }
}
...
impl PyPatternSubject {
    ...
    /// Create a pattern with Subject value and elements.
    ///
    /// Args:
    ///     subject (Subject): Subject instance to use as pattern value
    ...
    ///
    /// Returns:
    ///     PatternSubject: Pattern<Subject> instance with value and elements
    #[staticmethod]
    ...
    fn filter(&self, py: Python, predicate: &Bound<'_, PyAny>) -> PyResult<Vec<PyPatternSubject>> {
        let filtered = self.pattern.filter(|p| {
            let py_pattern = PyPatternSubject {
                pattern: p.clone(),
            };
    ...
    /// Find the first subpattern that satisfies the predicate.
    fn find_first(&self, py: Python, predicate: &Bound<'_, PyAny>) -> PyResult<Option<PyPatternSubject>> {
        ...
        Ok(found.map(|p| PyPatternSubject {
            pattern: p.clone(),
        }))
    }
    ...
    fn depth_at(&self) -> PyResult<PyPatternSubject> {
        ...
        Err(runtime_error_to_python("depth_at not yet implemented for PatternSubject"))
    }

    /// Decorate each position with subtree size.
    fn size_at(&self) -> PyResult<PyPatternSubject> {
        Err(runtime_error_to_python("size_at not yet implemented for PatternSubject"))
    }

    /// Decorate each position with path from root.
    fn indices_at(&self) -> PyResult<PyPatternSubject> {
        Err(runtime_error_to_python("indices_at not yet implemented for PatternSubject"))
    }
    ...
    fn __repr__(&self) -> String {
        format!("PatternSubject(identity={:?})", self.pattern.value.identity.0)
    }
}

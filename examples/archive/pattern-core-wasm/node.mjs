...
async function runExample() {
  ...
  const subject = new Subject("n", ["Person", "Employee"], {
    name: Value.string("Alice"),
    age: Value.int(30),
    salary: Value.decimal(75000.5),
  });
  ...
  const deepPattern = Pattern.pattern("a", [
    Pattern.pattern("b", [Pattern.pattern("c", [Pattern.point("d")])]),
  ]);
  const invalidResult = deepPattern.validate(rules);

  log(
    "12. Validation (Either-like)",
  ...
  const matchesPattern = numbers.matches(
    Pattern.pattern(1, [Pattern.point(2), Pattern.point(3)]),
  );

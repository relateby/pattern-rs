...
pub static TOPICS: &[TopicEntry] = &[]; // generated by build.rs
...
In this implementation, `build.rs` generates `OUT_DIR/topic_catalog.rs` from the markdown corpus in `skill-package/pato/reference/`, so you do not hand-maintain the topic list.
...
#[derive(Args, Debug)]
pub struct HelpArgs {
    /// Topic name (e.g., gram). Omit to list all topics.
...
The existing `skill_install/package.rs` copies the full `skill-package/pato/` tree. Since `reference/` is a subdirectory, the embedded bundle includes it automatically and the install code simply writes the embedded tree back out.

...
- [X] T002 [P] Author topic file `crates/pato/skill-package/pato/reference/gram.md` — include: one-sentence definition, syntax rules, semantic rules, 2–4 examples, related topics section
...
**Independent Test**: Run `pato help gram` and confirm output contains the topic content. Run `pato help no-such-topic` and confirm exit code 1, stderr contains an error message and the list of available topics.
...
**Checkpoint**: `pato help gram` works end-to-end. `pato help` and `pato help bad-topic` fail clearly with topic list. User Story 2 is independently testable.
...
**Independent Test**: Run `pato skill --scope project`, then confirm `.agents/skills/pato/reference/gram.md` and `.agents/skills/pato/reference/stdout-stderr-contracts.md` exist and are non-empty.
...
- [X] T016 [US3] Update `crates/pato/tests/skill_tests.rs`: assert that after a simulated install the installed files are present AND their content matches the corresponding `SKILL_BUNDLE` entries byte-for-byte; include an assertion for `reference/gram.md` and `reference/stdout-stderr-contracts.md` specifically (depends on T015)
...
5. **STOP and VALIDATE**: `pato help gram` works; `pato help` and bad topic fail clearly
6. Ship if ready

...
A library user wants to compute a value per element (e.g., a label or score) that depends on the values of the elements it structurally contains — for example, "depth from leaves" or "aggregated sub-element score." The fold processes elements in bottom-up containment order so each element receives already-computed results for its direct sub-elements. For cyclic annotation/other graphs, an iterative variant repeats until a convergence condition is met.

**Why this priority**: Paramorphism supports graph algorithms (enrichment, propagation, fixed-point iteration). It is the foundation for Pregel-style computation. Lowest priority in this feature because it builds on all previous view and query machinery.

**Independent Test**: Build a view with nodes, relationships, and an annotation-of-annotation chain. Run para-graph with a function that returns `max(sub_depths) + 1` or 0 if no sub-elements. Assert nodes get 0, relationships get 1 (containing nodes), and nested annotations get increasing depths. Delivers structure-aware per-element computation. For convergence, run para-graph-fixed and assert results stabilize.

**Acceptance Scenarios**:

1. **Given** a graph view and a function (query, pattern, sub_results) -> result, **When** the user runs para-graph, **Then** the result is a map from element identity to result; elements are processed in `topo_shape_sort` order (nodes → relationships → walks → annotations → other, with within-bucket topological sort for annotations and other), so each element’s sub-elements have already been processed when `f` is called.
2. **Given** a graph with annotation-of-annotation chains, **When** para-graph is run, **Then** inner annotations are processed before outer ones and each annotation receives its direct sub-element’s result in `sub_results`.
3. **Given** a graph with a dependency cycle within the annotation or other bucket, **When** para-graph is run, **Then** no error is raised, all elements appear in the result, and cycle members process with best-effort `sub_results` (the first-processed cycle member gets `sub_results = []`).
4. **Given** para-graph-fixed with a convergence predicate and initial value, **When** run on a view, **Then** the implementation iterates rounds using `topo_shape_sort` order until the predicate holds for all elements, and the final map is returned.
5. **Given** para-graph with a function that ignores sub-element results, **When** run on any view, **Then** the result map has one entry per element identity in the view; no elements are dropped.
...
- What happens when para-graph is run on a graph with dependency cycles within the GAnnotation or GOther bucket? Cycle members are processed in encountered order; the first-processed cycle member receives `sub_results = []` for the other cycle members (soft-miss); the second may find the first's result already in the accumulator. No error is raised and all elements appear in the result. Use para-graph-fixed for fixed-point semantics when cycles are present.
...
- **FR-012**: Library MUST provide a para-graph operation that takes a graph view and a function (query, pattern, sub_results) -> result, and returns a map from element identity to result; processing order MUST be `topo_shape_sort`: nodes first, then relationships, then walks, then annotations, then other (inter-bucket), with within-bucket topological sort (Kahn's algorithm) applied to GAnnotation and GOther buckets so nested annotations and other elements receive their direct sub-element results before being processed themselves; cycle members within a bucket are processed in encountered order with best-effort sub_results.

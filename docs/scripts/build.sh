...
cat > "${REPO_ROOT}/docs/public/reference/rust/index.html" << 'EOF'
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <title>Rust API Reference — pattern-rs</title>
  <style>body{font-family:sans-serif;max-width:600px;margin:4rem auto;padding:0 1rem}a{color:#3b82f6}</style>
</head>
<body>
  <h1>Rust API Reference</h1>
  <p>Documentation generated by <code>cargo doc</code>.</p>
  <ul>
    <li><a href="pattern_core/">relateby-pattern</a> — core Pattern&lt;V&gt; and Subject types</li>
    <li><a href="gram_codec/">relateby-gram</a> — Gram notation parser and serializer</li>
  </ul>
</body>
</html>
EOF
